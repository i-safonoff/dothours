...
**CityDistrict** (статический каталог): `key`, `building_family` (null = общая зона), `title`, `grid_x`, `grid_y`, `grid_w`, `grid_h`
`CityBuilding` получает: `district_id`, `position_x`, `position_y`, `rotation`, `variant`

Эндпоинт: `GET /city/districts`

Принятые решения:
- Каталог районов живёт в коде (`app/city_districts.py`), как и семейства
  зданий, но синхронизируется в таблицу `city_districts` — таблица нужна
  только чтобы `CityBuilding.district_id` был честным внешним ключом. Синк
  идемпотентный и делается на чтении, так что код и таблица не разъезжаются.
- Раскладка **детерминированная от `id` здания**: район — из семейства, а
  конкретный тайл, поворот и `variant` — из хеша `id`. Город выглядит
  одинаково на всех клиентах и после любого рестарта, при этом два города с
  одинаковым набором зданий не выглядят близнецами. Никакого состояния
  алгоритма раскладки хранить не нужно.
- Раскладка присваивается один раз, при создании здания, и больше не меняется:
  апгрейд уровня не двигает здание по карте.
- Здания, созданные до Этапа 7, доразмечаются лениво при чтении
  `GET /city/me` и `GET /companies/{id}/city`.
- `position_x/y` — абсолютные координаты сетки, а не смещение внутри района:
  фронту не приходится складывать их с `grid_x/grid_y` самому.
- В центре — общая площадь `plaza` без семейства; районы не пересекаются
